 ...
 */
/*
 * NOTE: all the data structures of this class are protected by the instance's
 * object monitor. To avoid deadlock with third party code, this lock is never
 * held when calling methods outside this class that may in turn use locks.
 */
class WorkerPool {
	/**
	...
	protected void endJob(Job job, IStatus result) {
		try {
			...
			// manager.getLockManager().removeLockCompletely(Thread.currentThread(),
			// job.getRule());
			// }
	...
	protected Job startJob(Worker worker) {
		...
		try {
			...
			if (job != null) {
				...
				// lock
				// if ((job.getRule() != null) && !(job instanceof ThreadJob)) {
				// //don't need to re-acquire locks because it was not recorded
				// in the graph
				...
				if (manager.sleepHint() < Job.T_INFINITE)
					jobQueued();
			}
		} finally {
